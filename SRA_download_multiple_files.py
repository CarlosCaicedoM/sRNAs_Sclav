...
#Summarize all the fastqc results with multiQC
MultiQC(fastqc_trimmed, "multiqc_cutadapt")
...
#%% MultiQC for fastp results

MultiQC(fastp_dir, "multiqc_fastp2")

#MultiQC can read the html files generated by fastp but it is unable to 
...
try:
  os.mkdir(os.path.join(cur_dir, "results_rna_seq", my_experiment, "Bowtie2_cutadapt"))
except OSError as error:
    print(error)
...
sorted_indices = np.lexsort((HT_concat.index, [x[1] for x in index_tuples]))

HT_concat = HT_concat.iloc[sorted_indices]
