...
def HTSeq_count(bam_files, bam_folder, folder_results, threads, type_data)
    try:
...
def apero_to_GFF(sRNAs_apero, seqid):
    ...
    # Generate sequential names for Column 3
    ...
    #gff['Column9'] = 'name='+gff['Column3']+'**'+sRNAs_apero['Position'].astype(str)+'-'+(sRNAs_apero['Position'] + sRNAs_apero['lg']).astype(str) +';Class=' + sRNAs_apero['Class'].astype(str)+', ' + sRNAs_apero['Comment'].astype(str)
...
def concat_gff(path_original, gff_apero):
    gff_original = pd.read_csv(path_original, comment='#', 
                               sep='\t', header=None)
...
try:
  os.mkdir(os.path.join(os.getcwd(), "results_rna_seq", my_experiment, "HTseq_APERO_CHR"))
except OSError as error:
    print(error)
...
for b in bam_files:  
    ...
    bam_file =  os.path.join(os.getcwd(), "results_rna_seq", my_experiment, "Bowtie2_CHR", b)
    output_htseq = os.path.join(os.getcwd(), "results_rna_seq",
                              my_experiment, "HTseq_APERO_CHR", b)  +".counts"

    command_htseq = ["htseq-count", "-f", "bam", "-t", "putative_sRNA", 
                     "-i", "name", "-n", "20", 
                     "-r", "pos", bam_file, file_gff_APERO_CHR]
